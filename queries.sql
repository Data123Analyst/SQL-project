...
select department , count(*) as total_count, sum(case when termdate<>'0000-00-00'and term_date<=curdate() then 1 else 0 end) as terminated_count
from hr
where age>=18
...
from( select year(hire_date) as year , count(*) as hires , sum(case when termdate<>'0000-00-00'and term_date<=curdate() then 1 else 0 end) as terminations
from hr
where age>=18
